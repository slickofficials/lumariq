...
class SensorySystem(context: Context) {
    ...
    // SAFEGUARD: Wraps vibration in Try-Catch to prevent crashes
    private fun safeVibrate(action: () -> Unit) {
        try {
            action()
        } catch (e: Exception) {
            // If hardware fails, we stay silent and alive.
        }
    }

    // SAFEGUARD: Tone Generator might fail if audio is busy
